...
def atom_expr(p):
    ...
    with p:
        return Node('ref', name=p(hier_name))
...
def slice_expr(p):
    ...
    with p:
        ...
        with p:
            ...
            p(ws)
        p(r'\]')
        return Node('slice-expr', callee=callee, lower_bound=lower_bound, upper_bound=upper_bound)
    return callee
...
def simple_type(p):
    ...
    return Node('simple-type', name=name, args=gen_args)
...
def _member_decl(p):
    ...
    with p:
        ...
        return name, type
    bounds = p(array_bounds)
    return name, Node('type', name='bit', bounds=[])
...
def def_decl(p):
    ...
    return Node('mod', name=name, mod=mod, port_maps=pms)
...
def top_decl(p):
    ...
    with p:
        ...
        return Node('interface', decls=decls)
    ...
    with p:
        ...
        for e in p(_indented, line_enumers):
            enumers.extend(e)
        return Node('enum', enumers=enumers)
    ...
    with p:
        ...
        return Node('struct', members=members)
    ...
    with p:
        ...
        return Node('module', ports=ports)
...
with open(r'test.bv', 'r') as fin:
    parsed = speg.peg(fin.read().replace('\r', ''), root)
